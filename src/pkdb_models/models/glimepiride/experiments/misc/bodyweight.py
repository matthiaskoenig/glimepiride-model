...
class BodyweightExperiment(GlimepirideSimulationExperiment):
    """Bodyweight-dependent simulation for glimepiride administration."""
        ...
        for substance, routes in self.routes.items():
            for route in routes:
                for bodyweight in self.bodyweights:
                    tcsims[f"{substance}_{route}_{bodyweight}"] = TimecourseSim(
                        Timecourse(
                            start=0,
                            end=24 * 60,  # [min]
                            steps=5000,
    ...
    def figures(self) -> Dict[str, Figure]:
        ...
        for substance, routes in self.routes.items():
            for route in routes:
                fig = Figure(
                    experiment=self,
                    sid=f"timecourse_po_bw",
                    num_rows=3,
                    num_cols=3,
                    name=f"Glimepiride - Bodyweight",
                )
                ...
                for ksid, sid in enumerate(sids):
                    for kval, bodyweight in enumerate(self.bodyweights):
                        plots[ksid].add_data(
                            task=f"task_{substance}_{route}_{bodyweight}",
                            xid="time",
                            yid=sid,
                            label=f"{bodyweight} kg",
                            color=self.bodyweight_colors[bodyweight],
                        )
                figures[fig.sid] = fig
        return figures
